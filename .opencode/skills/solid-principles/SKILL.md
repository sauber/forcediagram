...
SOLID is five design principles for maintainable, testable, and extensible
object-oriented code. Apply them when organizing modules, classes, and
functions.
...
- If you cannot describe what a module does in a single sentence without using
  "and", split it.
- Group **cohesive** behavior together; separate **different concerns** into
  distinct modules.
...
- Use **strategy pattern**, **polymorphism**, or **function composition** to
  vary behavior.
...
- A derived class should **not weaken** preconditions or **strengthen**
  postconditions of the base.
- If code that works with the base type breaks with a subclass, the design
  violates LSP.
...
> Depend on **abstractions**, not concretions. High-level modules should not
> depend on low-level modules.

- Inject dependencies (e.g. via constructor parameters) rather than
  instantiating them inside.
- Define interfaces/abstract types for external services so they can be swapped
  (testing, variants).
...
- **One module = one responsibility** (SRP). Keep files focused and small (under
  ~200 lines).
- **Export only what consumers need** — keep internals private.
- **Use function composition** for extensibility (OCP) instead of deep class
  hierarchies.
- **Inject collaborators** (DIP) rather than importing concrete implementations.
- **Prefer interfaces for external contracts** and types for internal shapes
  (ISP).
- Group related types, functions, and classes into directories by domain
  concern, not by role (e.g. `invoicing/` not `models/` + `services/`).

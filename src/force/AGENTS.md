...
- Containment: A node pulls inwards from parent edges (also from Canvas)
- Implosion: A node contracts onto itself to minimize size
- Gravity: A node seeks towards center of parent (also of Canvas)
- Embracement: A parent node pulls away from child nodes
- Alignment: Sibling nodes pull towards aligning edges, for example to have left
  edge at same position.
- Sizing: Sibling nodes pull towards having same size
- Grid: Edges of nodes pull towards canvas grid lines
- Overlap: Repulsion of overlapping nodes, overlapping links or overlapping of
  links and nodes.
